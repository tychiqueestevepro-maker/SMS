-- Add sending_days column to campaigns
ALTER TABLE public.campaigns 
  ADD COLUMN sending_days integer[] NOT NULL DEFAULT '{1,2,3,4,5}';

-- Update the send window check function to enforce sending_days
...
AS $$ 
  select (p_at at time zone campaign.timezone)::time >= campaign.send_window_start 
         and (p_at at time zone campaign.timezone)::time < campaign.send_window_end 
         and extract(isodow from (p_at at time zone campaign.timezone)) = ANY(campaign.sending_days)
  from public.campaigns as campaign where campaign.id = p_campaign_id; 
$$;

-- Update create_campaign_draft
CREATE OR REPLACE FUNCTION public.create_campaign_draft(p_workspace_id uuid, p_name text, p_steps jsonb, p_phone_number_id uuid DEFAULT NULL::uuid,
  ...
  p_sending_days integer[] DEFAULT '{1,2,3,4,5}'::integer[])
 RETURNS campaigns
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
