...
export async function GET(request: Request) {
  ...
  try {
    ...
    for (let index = 0; index < MAX_DISPATCH_PER_RUN; index += 1) {
      ...
      dispatched += 1;
      // A fail-closed validation can leave the same due recipient pending for
      // a later state change. Stop this invocation so it cannot hot-loop on
      // that single message.
    ...
    return NextResponse.json({ dispatched, inboundReconciled, reconciled });
  } catch {
    logServerEvent(
      "error",
